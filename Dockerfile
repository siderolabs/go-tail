...
COPY ./tail.go ./tail.go
COPY ./tail_test.go ./tail_test.go
...
RUN FILES="$(goimports -l -local github.com/siderolabs/go-tail .)" && test -z "${FILES}" || (echo -e "Source code is not formatted with 'goimports -w -local github.com/siderolabs/go-tail .':\n${FILES}"; exit 1)
